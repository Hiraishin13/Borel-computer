...
import { ASSEMBLY_SKU, ASSEMBLY_FEE } from './constants'
...
/** Peuple la base : admin, service, catalogue, PC configurés, promos, config boutique.
 * Idempotent (upserts). Ne crée l'admin que s'il n'existe pas. */
export async function runSeed(): Promise<SeedResult> {
  ...
  const promos = [
    { code: 'BIENVENUE10', type: 'percentage', value: 10, description: 'Première commande', maxUses: 0, minPurchase: 150, applicableCategories: [] as string[] },
    { code: 'SETUP50', type: 'fixed', value: 50, description: 'Remise périphériques', maxUses: 200, minPurchase: 300, applicableCategories: ['peripheriques'] },
  ]
  for (const p of promos) {
    await Promo.updateOne(
      { code: p.code },
      { $set: { ...p, validFrom: new Date('2026-01-01'), validUntil: new Date('2026-12-31'), active: true } },
      { upsert: true },
    )
  }
